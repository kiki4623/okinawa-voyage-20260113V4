...
export const Translator: React.FC = () => {
  ...
  const handleTranslate = useCallback(async (text: string, targetLang: 'ja' | 'zh' = 'ja') => {
    ...
    try {
      ...
      setResult({ japanese: "Error", romaji: "Error", english: "Error", chinese: "翻譯發生未預期錯誤", error: e.message });
    } finally {
      setLoading(false);
    }
  }, [loading]);
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 space-y-12">
      ...
      <section className="bg-white rounded-[2.5rem] border border-[#F0EFEA] shadow-sm relative overflow-hidden flex flex-col min-h-[500px]">
        ...
        <div className={`flex-1 p-8 bg-[#8B1D3D]/5 flex flex-col items-center justify-center text-center transition-all duration-500 relative ${isFaceToFace ? 'rotate-180' : ''}`}>
              ...
              {result.error ? (
                <div className="flex flex-col items-center gap-3 text-red-500">
                 ...
                 <>
                   ...
                   {result.romaji && (
                     <p className="text-sm font-medium text-[#A09E97] font-cormorant italic">
                       {result.romaji}
                     </p>
                   )}
                 </>
          ...
          <button 
            ...
            className={`absolute bottom-6 right-6 p-4 rounded-full shadow-lg transition-all active:scale-95 z-20 ${
          ...
          <div>
            ...
            {result?.chinese && !loading && !result.error && (
              <div className="mb-6 p-5 bg-[#8B1D3D]/5 rounded-2xl border border-[#8B1D3D]/10 animate-in slide-in-from-top-2">
                ...
                <p className="text-2xl font-noto-serif font-black text-[#8B1D3D] leading-tight">
                  {result.chinese}
                </p>
              </div>
            )}
            ...
            <div className="relative mb-6">
              <textarea 
                className="w-full bg-[#F5F4F0] border border-[#F0EFEA] rounded-2xl p-5 text-sm font-bold focus:outline-none focus:border-[#8B1D3D] transition-all min-h-[100px] resize-none pr-16"
              ...
              <div className="absolute bottom-4 right-4 flex gap-2">
                 <button 
                    ...
                    className={`p-3 rounded-xl transition-all active:scale-95 ${
                      isRecording && recordingTarget === 'me'
                        ? 'bg-red-500 text-white animate-pulse shadow-md'
                        : 'bg-white text-[#A09E97] border border-[#F0EFEA] hover:text-[#8B1D3D]'
                    }`}
                  >
                    {isRecording && recordingTarget === 'me' ? <MicOff className="w-5 h-5" /> : <Mic className="w-5 h-5" />}
                  </button>
                  <button 
      ...
      <section className="space-y-8">
        <div className="flex items-baseline gap-3 px-2">
           <h3 className="text-3xl font-noto-serif font-bold text-[#2D2D2D]">求生日語</h3>
        </div>
            ...
            return (
              <button 
                ...
                className={`flex items-center gap-2.5 px-6 py-2.5 rounded-full whitespace-nowrap font-bold text-[11px] transition-all ${
                  isActive 
                    ? 'bg-[#8B1D3D] text-white shadow-md' 
                    : 'bg-white text-[#A09E97] border border-[#F0EFEA] hover:bg-[#F5F4F0]'
                }`}
              >
          ...
          {COMMON_PHRASES[activeCategory].list.map((phrase, idx) => (
            ...
            >
              <div className="flex-1 space-y-1.5">
                ...
                <p className="text-3xl font-noto-serif font-black text-[#2D2D2D] leading-snug tracking-tight">
                  {phrase.jp}
                </p>
                <p className="text-lg font-cormorant font-bold text-[#B0B0B0] tracking-wide italic">
                  {phrase.romaji}
                </p>
              </div>
